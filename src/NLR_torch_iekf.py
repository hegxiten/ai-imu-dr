...
import numpy as np
import os
from NLR_iekf import NUMPYIEKF
import pickle
from NLR_utils import get_normalize_u
...
class TORCHIEKF(torch.nn.Module, NUMPYIEKF):
    ...
    def init_run(self, dt, v0, N, pose0):
            Rot, v, p, b_omega, b_acc, Rot_c_i, t_c_i = self.init_saved_state(dt, N)
            Rot[0] = self.from_rpy(pose0[0], pose0[1], pose0[2])
            v[0] = v0
    ...
    def init_saved_state(self, dt, N):
    ...
    def propagate_cov(self, P, Rot_prev, v_prev, p_prev, dt):
    ...
    def load(self, args):
        ...
        if os.path.isfile(path_iekf):
            ...
            print("IEKF nets loaded")
        else:
            print("IEKF nets NOT loaded")
            quit()
        self.set_normalize_u(args)

    def set_normalize_u(self, args):
        self.u_loc, self.u_std = get_normalize_u(args)
